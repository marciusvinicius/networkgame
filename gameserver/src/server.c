...
void init_player(Player *player, int id)
{
    ...
    printf("Initialized player %d at position (%d, %d) with color %d\n",
           id, player->x, player->y, player->color_index);
}
...
void broadcast_player_positions(void)
{
    ...
    for (int i = 0; i < MAX_PLAYERS; i++)
    {
        if (players[i].active)
        {
            pkt.players[pkt.player_count].id = players[i].id;
            pkt.players[pkt.player_count].x = players[i].x;
            pkt.players[pkt.player_count].y = players[i].y;
            pkt.players[pkt.player_count].color_index = players[i].color_index; // Include color information
            pkt.players[pkt.player_count].is_active = players[i].active;        // Include active status
    ...
    // Create the packet
    ENetPacket *epkt = enet_packet_create(&pkt,
                                          sizeof(unsigned char) * 2 +
                                              sizeof(struct { unsigned char id, x, y, color_index; }) * pkt.player_count,
    ...
    // send tile chunk to all players
    for (int i = 0; i < MAX_PLAYERS; i++)
    {
        if (players[i].active)
        {
            int chunk_x = players[i].x / CHUNK_SIZE;
            int chunk_y = players[i].y / CHUNK_SIZE;
            send_tile_chunk(&server->peers[i], chunk_x, chunk_y);
        }
    }
}
...
bool load_map_from_file(const char *filename)
{
    ...
    while (fgets(line, sizeof(line), file) && y < VIEWPORT_HEIGHT)
    {
        ...
        while (token && x < VIEWPORT_WIDTH)
        {
            ...
            game_map[x][y].walkable = (tile_value == 1); // Only tiles with value 1 are walkable

            // Debug output for the first few tiles
            if (x < 3 && y < 3)
            {
                printf("Loaded tile at (%d, %d): id=%d, walkable=%d\n",
                       x, y, game_map[x][y].tile_id, game_map[x][y].walkable);
            }
    ...
    printf("Map summary: %d/%d tiles are walkable (%.1f%%)\n",
           walkable_count, total_tiles, (float)walkable_count / total_tiles * 100);
...
void handle_client_connection(ENetEvent *event)
{
    ...
    ENetPacket *id_epkt = enet_packet_create(&id_pkt, sizeof(id_pkt), ENET_PACKET_FLAG_RELIABLE);
    enet_peer_send(event->peer, 0, id_epkt);

    printf("Sent player ID %d to new client\n", player_id);

    // Send the initial tile chunk to the client
    int chunk_x = player_id % (VIEWPORT_WIDTH / CHUNK_SIZE);
    int chunk_y = player_id / (VIEWPORT_WIDTH / CHUNK_SIZE);
...
void process_move(ENetPeer *peer, MovePacket *pkt)
{
    ...
    {
        printf("Player %d move rejected: new position (%d, %d) is not walkable (tile_id=%d)\n",
    ...
    if (old_chunk_x != new_chunk_x || old_chunk_y != new_chunk_y)
    {
        printf("Player %d moved to a new chunk: (%d, %d) -> (%d, %d)\n",
               player_id, old_chunk_x, old_chunk_y, new_chunk_x, new_chunk_y);
        ...
        if (new_x % CHUNK_SIZE < CHUNK_SIZE / 2 && new_y % CHUNK_SIZE < CHUNK_SIZE / 2)
        {
        ...
        else if (new_x % CHUNK_SIZE >= CHUNK_SIZE / 2 && new_y % CHUNK_SIZE < CHUNK_SIZE / 2)
        {
        ...
        else if (new_x % CHUNK_SIZE >= CHUNK_SIZE / 2 && new_y % CHUNK_SIZE >= CHUNK_SIZE / 2)
        {
            ...
            if (new_chunk_x < VIEWPORT_WIDTH / CHUNK_SIZE - 1 && new_chunk_y < VIEWPORT_HEIGHT / CHUNK_SIZE - 1)
            {
                send_tile_chunk(peer, new_chunk_x + 1, new_chunk_y + 1);
            }
        }
