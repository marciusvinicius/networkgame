...
void update_player_positions(const PlayerPositionsPacket *packet)
{
    ...
    for (int i = 0; i < packet->player_count; i++)
    {
        ...
        players[i].color_index = packet->players[i].color_index;
        players[i].active = packet->players[i].is_active;
    }
...
void handle_tile_chunk(const TileChunkPacket *pkt)
{
    ...
    printf("Updated %d tiles in chunk (%d, %d)\n", CHUNK_SIZE * CHUNK_SIZE, pkt->chunk_x, pkt->chunk_y);
}
...
void draw_tiles()
{
    ...
    for (int y = 0; y < VIEWPORT_HEIGHT; y++)
    {
        for (int x = 0; x < VIEWPORT_WIDTH; x++)
        {
            ...
            // Draw a grid line
            DrawRectangleLines(x * TILE_SIZE, y * TILE_SIZE, TILE_SIZE, TILE_SIZE, (Color){50, 50, 50, 100});
        }
    }
...
bool init_window()
{
    InitWindow(VIEWPORT_WIDTH * TILE_SIZE, VIEWPORT_HEIGHT * TILE_SIZE, "Game Client");
    SetTargetFPS(60);
    return true;
}
